...
public class MessageReceived(ILogger<MessageReceived> logger, IHostEnvironment environment)
{
    ...
    public SignalRMessageAction Run(
        [EventHubTrigger("%AzureIotHubEventHub:Name%", Connection = "AzureIotHubEventHub:ConnectionString", IsBatched = false)]
        EventData evt)
    {
        logger.LogInformation("Event Body: {body}", evt.Body);
        logger.LogInformation("Event Content-Type: {contentType}", evt.ContentType);

        var action = new SignalRMessageAction("messages/newMessage");
        return action;
    }
}
